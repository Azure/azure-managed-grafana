...
Some of the most useful Azure data sources don't authenticate with a long-lived API key — they authenticate with a short-lived **Microsoft Entra ID access token**. Azure Monitor managed Prometheus and Azure Databricks both work this way, and an Entra ID token is only valid for about an hour. Paste one into a Grafana data source by hand and your dashboards work great — until the token expires and every panel starts returning auth errors.
...
```
              ...
              └─────────────────┬─────────────────┘
               ...
               ▼                                 ▼
   ┌───────────────────────┐         ┌───────────────────────┐
   │ Prometheus data source│         │ Databricks data source│
   │ Authorization header  │         │ PAT / Token field     │
   └───────────┬───────────┘         └───────────┬───────────┘
               ▼                                 ▼
   ┌───────────────────────┐         ┌───────────────────────┐
   │ Azure Monitor         │         │ Azure Databricks      │
   │ workspace (Prometheus)│         │ SQL warehouse         │
   └───────────────────────┘         └───────────────────────┘
```
...
Because the syncer manages the credential, Grafana itself never has to know how to mint or refresh an Entra ID token for these data sources — it just sends the bearer credential the syncer last wrote.
...
| Data source | Token audience (Entra ID resource the token is minted for) | Where the syncer writes the token |
| ----------- | ---------------------------------------------------------- | --------------------------------- |
...
- The backing Azure resource for whichever data source you're syncing (an Azure Monitor workspace and/or an Azure Databricks workspace), as described below.
...
- **The managed identity needs backend access, and it's the same identity for every data source.** An instance has a single managed identity, so that one identity must hold every grant: the **Monitoring Data Reader** role on the Azure Monitor workspace for Prometheus, and workspace service-principal registration plus `CAN_USE` on the SQL warehouse for Databricks. A fresh token is useless if the identity can't read the backend.
